...
**Meeting Transcribe Agent** は、「マルチモーダル動画処理（YouTube URL / ローカル動画）」と「高精度音声2層構造パイプライン」を兼ね備えた、包括的な会議インテリジェンス＆インタラクティブ文字起こし AI エージェントです。自治体の市政会議、多国籍チームの技術定例会、インタビューや法務証言録取など、「ミリ秒単位の正確なタイムスタンプ」「高精度な話者分離」「構造化された議事録作成」が求められる専門的な現場向けに設計されています。
...
* **🎙️ 音声モード (`~1x` 基準消費)**：
  - **消費量目安**：音声は1秒あたり約32トークン（1時間の音声で数万〜十数万トークン程度）。
  - **位置づけ**：**最も経済的でトークンを節約**。画面や席札が不要な日常の録音、インタビュー、ポッドキャストに最適。
* **🤖 Agentic Video Understanding (`~2x` 消費)**：
  - **消費量目安**：音声モードの**約2倍程度**。
  - **位置づけ**：モデルが必要な瞬間のみ動的ツール呼び出しでフレームをサンプリングし、思考キャッシュを活用。数時間の長編動画やスライド密度の高い技術会議に最適。
* **⚡ 静的動画モード (`~3x` 消費)**：
  - **消費量目安**：音声モードの**約3倍程度**。
  - **位置づけ**：デフォルトで1秒あたり1フレーム（1 FPS）をサンプリング。生成速度が極めて高速（約数十秒で完了）で、ネームプレートやスライドを100%認識可能。一般的な長さの公開講演や市政会議に最適。
