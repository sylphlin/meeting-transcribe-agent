...
**Meeting Transcribe Agent**는 "멀티모달 비디오 처리 (YouTube URL 및 로컬 영상)"와 "고정밀 순수 오디오 2계층 파이프라인"을 모두 갖춘 올인원 회의 인텔리전스 및 대화형 전사 AI 에이전트입니다. 공공 시정 회의, 글로벌 테크 싱크업, 심층 인터뷰 및 법적 증언 녹취 등 "밀리초 단위의 정확한 타임스탬프", "화자 분리 (Diarization)", "구조화된 회의록 요약"이 필수적인 전문 업무 환경을 위해 설계되었습니다.
...
* **🎙️ 순수 오디오 모드 (`~1x` 기준 소비량)**:
  - **대략적 소비량**: 초당 약 32 토큰 (1시간 기준 수만~십수만 토큰 수준).
  - **포지셔닝**: **가장 경제적이며 토큰을 극대화하여 절약**. 영상이나 명패가 없는 일상 녹음, 인터뷰, 팟캐스트에 적합.
* **🤖 Agentic Video Understanding (`~2x` 소비량)**:
  - **대략적 소비량**: 순수 오디오 기준 **약 2배 수준**.
  - **포지셔닝**: 모델이 동적 도구 호출을 통해 핵심 프레임만 능동적으로 탐색하고 씽킹 캐시를 활용합니다. 수 시간 길이의 영상이나 슬라이드가 빽빽한 기술 컨퍼런스에 최적.
* **⚡ 정적 비디오 모드 (`~3x` 소비량)**:
  - **대략적 소비량**: 순수 오디오 기준 **약 3배 수준**.
  - **포지셔닝**: 기본적으로 초당 1프레임(1 FPS)을 샘플링합니다. 생성 속도가 수십 초 내외로 매우 빠르며, 명패와 슬라이드를 100% 인식할 수 있습니다.
...
* **듀얼 트랙 비동기 병렬 처리**: "요약/결정사항"과 "장문 전사 교정"을 병렬로 분리 생성하여 응답 대기 시간을 단축.
