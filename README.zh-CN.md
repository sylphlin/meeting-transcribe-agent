...
**Meeting Transcribe Agent** 是一套具备“多模态视频处理（YouTube / 本地视频）”与“纯音频高精度双层架构”的全方位会议智能与交互式转录 Agent。专为政务市政主管会议、跨国技术周会以及访谈法务存证等高度要求“时间戳绝对精准”、“发言人切分”与“结构化决策记录”的专业场景打造。
...
* **🎙️ 纯音频模式 (`~1x` 基准消耗)**：
  - **大致消耗**：纯语音音频每秒约 32 tokens（一小时音频约在数万至十余万 Tokens 区间）。
  - **经验定位**：**成本最经济、最省 Token**。适合无画面、无桌牌之一般日常录音、访谈与播客。
* **🤖 Agentic Video Understanding (`~2x` 消耗)**：
  - **大致消耗**：约为纯音频的 **2 倍左右**。
  - **经验定位**：模型通过动态工具调用主动探索关键画面，只在需要时抽样视频帧并结合思考缓存。适合长达数小时的长篇视频、图表密集，需要深度多轮推理的公开会议。
* **⚡ 静态视频模式 (`~3x` 消耗)**：
  - **大致消耗**：约为纯音频的 **3 倍左右**。
  - **经验定位**：默认每秒固定抽样视频帧（1 FPS），生成速度极快（约数十秒即可完成），且能 100% 识别桌牌姓名与演示文稿内容，适合一般长度的公开演讲、市政会议与研讨会。
...
- 👁️ **视觉名牌与幻灯片辅助识别**：利用视频画面上的字卡、背板、演示标题，全自动推导真实人名与职务。
- 🎙️ **高精度发言人区分与逐字转录**：清楚标记每位与会者的发言起止与真实姓名职务。
- ✍️ **前后文理解与语意流畅化**：超越死板字典转换，通过 LLM 前后文理解自动校正同音错字（如专有名词、头衔），并使转录内容符合自然语意表达。
...
* **双轨异步并行 (Dual-Track Concurrency)**：
  * 将“核心决策摘要”与“长篇逐字稿修复”拆分为两个独立轨道并行生成，彻底解决长篇文本顺序输出的阻塞问题。
...
## 🏗️ 核心处理流水线 (Pipeline Architecture)
...
- **纯语音文件**（`.mp3`, `.m4a`, `.wav`, `.aac`, `.flac`）或加入 `--extract-audio` 标志者：自动分流至 **🎙️ 纯音频双层管线**。
...
**本地离线备用模式（可选）**：
```bash
# Apple Silicon (M1/M2/M3/M4) GPU 加速
...
> [!NOTE]
> 作为 AI Agent Skill 使用时，您无需手动执行任何终端命令，直接在对话框中向 Agent 提出需求即可！
...
```bash
# 云端纯音频默认模式
python3 meeting_transcribe.py "会议录音.mp3"

# 本地离线备用执行（Apple Silicon GPU / Sherpa-ONNX）
python3 meeting_transcribe.py "会议录音.mp3" --engine whisper --whisper-backend auto
```

### 包含会议大纲与指定摘要语言
```bash
python3 meeting_transcribe.py "会议录音.mp3" --outline "agenda.txt" --summary-language en
```
