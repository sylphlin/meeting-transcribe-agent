...
**Meeting Transcribe Agent** 是一套具備「多模態視訊處理（YouTube / 本地影片）」與「純音訊高精度雙層架構」的全方位會議智慧與互動式轉譯 Agent。專為公務市政主管會議、跨國技術週會以及訪談法務存證等高度要求「時間戳絕對精準」、「發言人切分」與「結構化決策記錄」的專業場景所打造。
...
* **🎙️ 純音訊模式 (`~1x` 基準消耗)**：
  - **大致消耗**：純語音音訊每秒約 32 tokens（一小時音訊約在數萬至十多萬 Tokens 區間）。
  - **經驗定位**：**成本最經濟、最省 Token**。適合無畫面、無座牌之一般日常純錄音檔、訪談與 Podcast。
* **🤖 Agentic Video Understanding (`~2x` 消耗)**：
  - **大致消耗**：約為純音訊的 **2 倍左右**。
  - **經驗定位**：模型透過動態工具調用主動探索關鍵畫面，只在需要時抽樣訊框並結合思考快取。適合數小時長篇視訊、簡報圖表密集，需要深度多輪推論的公開會議。
* **⚡ 靜態視訊模式 (`~3x` 消耗)**：
  - **大致消耗**：約為純音訊的 **3 倍左右**。
  - **經驗定位**：預設每秒固定取樣訊框（1 FPS），產出速度極快（約數十秒即可完成），且能 100% 辨識桌牌姓名與投影片內容，適合一般長度的公開演講、市政會議與研討會。
...
2. **零依賴互動式 HTML 播放器**：存檔為 `<檔名>_player.html`：
   - **YouTube 輸入**：右下角自動嵌入畫中畫可縮放之 YouTube 視訊視窗，點擊逐字稿秒數精確跳轉 YouTube 播放位置。
